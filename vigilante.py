...
def detectar_movimientos(snaps, liq, estado):
    """
    Compara con el estado anterior y devuelve la lista de avisos NUEVOS
    (movimientos interesantes que no habiamos avisado ya).
    """
    avisos = []
    filas_registro = []
    ...
    for s in snaps:
        ...
        # 1. Cerca de soporte fuerte CON barrida (senal de mas calidad)
        for sup in (s.get("supports") or []):
            if sup["tipo"] in ("minimo repetido", "origen del ultimo tramo",
                ...
                clave = f"{nombre}-soporte-{sup['nivel']}"
                nuevo_visto[clave] = True
                if clave not in visto:
                    barrida = sup.get("trampa")
                    marca = " CON barrida (mas fiable)" if barrida else ""
                    avisos.append((
                        4 if barrida else 3,
                        f"Cerca de soporte: {nombre}",
                        f"{nombre} a {sup['dist_pct']}% de un soporte fuerte "
                        f"({sup['nivel']}, {sup['tipo']}){marca}. Stop bajo {sup['stop']}.",
                    ))
                    filas_registro.append({
                        "modo": "vigilante", "activo": nombre,
                        "tipo_senal": "cerca de soporte", "precio": precio,
                        "nivel": sup["nivel"], "stop": sup["stop"],
                        "barrida": barrida, "detalle": sup["tipo"],
                    })
                break

        # 2. Soporte recien perforado (barrida o ruptura)
